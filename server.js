...
app.get('/api/init', async (req, res) => {
    try {
        ...
        console.log(`✅ Init data: price=${ticker.c}, klines=${klines.length}`);
        res.json({
...
// Rota explícita para manifest.json
...
app.get('/sw.js', (req, res) => {
    ...
    console.log('📄 Serving sw.js from:', swPath);
...
app.get('/icons/:filename', (req, res) => {
    const iconPath = path.join(__dirname, 'public', 'icons', req.params.filename);
...
// Fallback para frontend - serve index.html para todas as outras rotas
app.get('*', (req, res) => {
...
server.listen(PORT, () => {
    ...
    if (!fs.existsSync(publicDir)) {
        console.error('❌ Pasta public não existe!');
    } else {
        console.log('✅ Pasta public existe');
        if (fs.existsSync(manifestPath)) {
            console.log('✅ manifest.json existe');
        } else {
            console.error('❌ manifest.json NÃO existe em:', manifestPath);
        }
